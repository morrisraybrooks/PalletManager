...
object StationUtils {
    ...
    /**
     * Normalize station number to the full format used in the database
     * Converts various input formats to "03-XX-XX-01" format
     */
    fun normalizeStationNumber(input: String): String {
        ...
        // If it's already in full format (03-XX-XX-01), keep it
        if (cleaned.matches(Regex("03-\\d{2}-\\d{2}-01"))) {
            Log.d("StationUtils", "normalizeStationNumber: Already in full format = $cleaned")
            return cleaned
        }

        // Handle various input formats and convert to full format
        when {
            // Format: "3-57-15-1" -> "03-57-15-01"
            cleaned.matches(Regex("3-\\d{2}-\\d{2}-1")) -> {
                val parts = cleaned.split("-")
                val result = "03-${parts[1]}-${parts[2]}-01"
                Log.d("StationUtils", "normalizeStationNumber: Converted $cleaned to $result")
                return result
            }

            // Format: "57-15" -> "03-57-15-01"
            cleaned.matches(Regex("\\d{2}-\\d{2}")) -> {
                val parts = cleaned.split("-")
                val result = "03-${parts[0]}-${parts[1]}-01"
                Log.d("StationUtils", "normalizeStationNumber: Converted $cleaned to $result")
                return result
            }

            // Format: "5715" -> "03-57-15-01"
            cleaned.length == 4 && cleaned.all { it.isDigit() } -> {
                val aisle = cleaned.substring(0, 2)
                val station = cleaned.substring(2, 4)
                val result = "03-$aisle-$station-01"
                Log.d("StationUtils", "normalizeStationNumber: Converted $cleaned to $result")
                return result
            }
    ...
    /**
     * Validate station number format - accepts various input formats
     */
    fun isValidStationNumber(input: String): Boolean {
        val cleaned = input.trim()

        return when {
            // Full format: "03-57-15-01"
            cleaned.matches(Regex("03-\\d{2}-\\d{2}-01")) -> true
            // User format: "3-57-15-1"
            cleaned.matches(Regex("3-\\d{2}-\\d{2}-1")) -> true
            // Simplified format: "57-15"
            cleaned.matches(Regex("\\d{2}-\\d{2}")) -> true
            // Numeric format: "5715"
            cleaned.length == 4 && cleaned.all { it.isDigit() } -> true
            else -> false
        }
    }
